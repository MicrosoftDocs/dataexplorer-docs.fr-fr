...
|[avg()](avg-aggfunction.md)|Valeur moyenne retourne dans le groupe|
...
* Remarque : les fonctions d’interpolation par défaut prennent `null` la valeur manquante. Par conséquent, il est recommandé de spécifier `default=` *double*( `null` ) dans `make-series` si vous avez l’intention d’utiliser des fonctions d’interpolation pour la série. 
...
L’utilisation `kind=nonempty` de dans produira `make-series` des résultats non vides des valeurs par défaut :
